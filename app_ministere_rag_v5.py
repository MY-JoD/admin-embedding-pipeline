...
/* FIX: Couleur du texte dans les inputs de la sidebar (URL) */
section[data-testid="stSidebar"] div[data-testid="stTextInput"] input {
  color: #0A1428 !important; /* Texte noir/bleu foncé */
...
def preprocess_question_llm(llm, question: str) -> dict:
    """
    Retourne un dict : {lang, corrected, final}
    - lang : 'fr'/'en'/'other'
    - corrected : correction orthographe si FR
    - final : requête à envoyer au retriever (FR corrigé ou traduit FR)
    """
    q = (question or "").strip()
    if not q:
        return {"lang": "other", "corrected": "", "final": ""}

    # Prompt ultra strict : JSON only
    pre_prompt = f"""
Tu es un préprocesseur de requêtes pour un système RAG.

Tâches:
1) Détecte la langue principale de la requête: fr / en / other.
2) Corrige l'orthographe/ponctuation sans changer le sens.
3) Si la langue n'est pas fr, traduis la requête en français.

Contraintes:
- Réponds UNIQUEMENT avec un JSON valide sur une seule ligne.
- Clés attendues: lang, corrected, translated_to_fr
- Pas de texte en dehors du JSON.

Requête: {q}
""".strip()

    resp = llm.invoke(pre_prompt)
    txt = resp.content if hasattr(resp, "content") else str(resp)

    m = re.search(r"\{.*\}", txt, re.DOTALL)
    if not m:
        return {"lang": "other", "corrected": q, "translated_to_fr": q, "final": q}
    ...
    lang = (data.get("lang") or "other").strip().lower()
    corrected = (data.get("corrected") or q).strip()
    translated = (data.get("translated_to_fr") or corrected).strip()

    final = corrected if lang == "fr" else translated
    return {"lang": lang, "corrected": corrected, "translated_to_fr": translated, "final": final}
...
def rag_answer(question: str, retriever, llm, max_sources: int):
    # 1) Préprocess via LLM
    prep = preprocess_question_llm(llm, question)
    final_q = prep.get("final") or question

    # 2) Retrieval sur la requête nettoyée/traduite
    docs = retriever.invoke(final_q)[:max_sources]
    context_str, sources_str = format_sources(docs, max_chars=600)

    allowed = " ".join([f"[S{i}]" for i in range(1, len(docs) + 1)]) or "[S1]"
    final_prompt = prompt.format(context=context_str, question=final_q, allowed_citations=allowed)
    resp = llm.invoke(final_prompt)
    answer = resp.content if hasattr(resp, "content") else str(resp)
    return answer, sources_str, docs, prep
...
with st.sidebar:
    ...
    st.info("⚠️ Après ajout ou modification d’une source (JSONL, PDF ou URL), relancez « Charger / Construire l’index ».")

    has_jsonl = (jsonl_upload is not None)
    has_pdf = (pdf_uploads is not None and len(pdf_uploads) > 0)
    has_url = (use_url and url_input.strip() != "")

    build_btn = st.button(
        "🚀 Charger / Construire l’index",
        use_container_width=True,
        disabled=not (has_jsonl or has_pdf or has_url),
    )
...
if build_btn:
    ...
    pdf_bytes = b"".join([pdf.getvalue() for pdf in pdf_uploads]) if pdf_uploads else b""
    url_bytes = url_clean.encode("utf-8") if url_clean else b""

    cid = sha256_bytes(b + pdf_bytes + url_bytes + salt)
        ...
        if not st.session_state.ready:
            ...
            with st.chat_message("assistant"):
                with st.spinner("Recherche + génération…"):
                    answer, sources_str, used_docs, prep = rag_answer(
                        question=user_q,
                        retriever=st.session_state.retriever,
                        llm=st.session_state.llm,
                        max_sources=max_sources,
                    )
                    st.session_state["last_prep"] = prep
                    st.session_state["last_user_q"] = user_q
                    st.markdown(answer)
...
with right:
    ...
    if "last_sources" in st.session_state:
        ...
        if st.session_state.get("is_admin", False):
            ...
            # 1) Pré-traitement (admin)
            if "last_prep" in st.session_state:
                with st.expander("Pré-traitement requête (admin)", expanded=False):
                    prep = st.session_state["last_prep"]
                    st.markdown(f"**Question originale :** {st.session_state.get('last_user_q','')}")
                    st.markdown(f"**Langue détectée :** `{prep.get('lang','NA')}`")
                    st.markdown(f"**Question corrigée :** {prep.get('corrected','')}")
                    if prep.get("lang") != "fr":
                        st.markdown(f"**Traduction FR :** {prep.get('translated_to_fr','')}")
                    st.markdown(f"**Requête envoyée au retriever :** {prep.get('final','')}")

            # 2) Passages utilisés (admin)  ✅ (c’était ça qui manquait)
